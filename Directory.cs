...
namespace OS_project2
{
    class Directory : Directory_Entry
    {
        ...
        public Directory (string namefile, byte attributefile,int firstClustumfile,int size, Directory p)
            :base
            (namefile,
             attributefile,
             firstClustumfile,
        ...
        public void writeDirectory()
        {
            ...
            for (int i = 0; i < allbytes.Count; i++)
            {
                if (fatIndex != -1)
                {
                    Virtual_Disk.writeBlock(allbytes[i], fatIndex);
        ...
        {
            int index = searchDirectory(new string(d.fileorDirName));
            if (index != -1)
            {
